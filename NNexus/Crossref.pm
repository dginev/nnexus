...
#
use vars qw{%LINKTAGS $menuscript};
...
sub crossReferenceLaTeX {
	...
	my $matches = doAutomaticLinking( $nonmath, \@user_escaped, $class, $fromid );
...
#				print "Traversing down: " .  $$i->tag(). "class = " .$$i->attr('class'). "\n" ;
				linkHTMLTree( $i, $domain, $syns, $fromid, $class );
			}
		} else {
		...
		#	$$i = $text;
			push @matches, doAutomaticLinking( $text, $syns, $class, $fromid );
			push @textref, $i;
		}
	}
...
# forks anyway.
	...
	linkHTMLTree( \$root, $domain, $syns, $fromid, $class );
	...
	for ( my $i = 0; $i < @matches; $i++ ) {
		...
		foreach my $pos (sort {$b <=> $a} keys %$match) {
			...
			push @scriptMenuCode, 
				substituteHTMLLinks( $objects, \@ltext,
					$pos, $pos+$length-1, $texttolink, $priorities);
#print "JAMESG :";
#print Dumper($objects);
			my $domainid = getdomainid($domain);
			my $linktarget = $objects->{$domainid}[0][0];
			my $lnk = getlinkstring( $linktarget, $texttolink );
...
#	push @$nolink, @blacklist;
	...
	if ( $mode eq 'l2h' ) {
		return crossReferenceLaTeX($domain, $text, $nolink, $fromid, $class);
	} elsif( $mode eq 'html' ) {
		print "LINKING IN HTML MODE\n"	if $DEBUG;
		return crossReferenceHTML( $domain, $text, $nolink, $fromid, $class);
	} else {
...
#		my $candidates = $terms->{$fw}->{$matchterms};
		my $finals = disambiguate($candidates, $matchterms, $class, $fromid);
		print Dumper( $finals );
...
#		my ($left, $right) = outertags($tags);
	...
	#	print "Making link string for " . Dumper($objects) . "\n";
		...
		#
		my $lnk = getlinkstring( $objects, $texttolink );
...
sub getlinkstring{ 
	...
	my $object = getobjecthash( $objectid );
...
#		print "making link to $domainnick\n";
		if ( $first == 0 ) {
			my $htmlonly = "\n" . '\begin{rawhtml}' . "\n";
			$htmlonly .= getmenulinkstring( $id, $texttolink, $nummenus, $size );
		...
		push @optionallinks, getlinkstring( $id, $domainnick );
		print "LEAVE loop";
	}
	my %menuinfo = ();
...
sub substituteHTMLLinks {
	...
	foreach my $k ( @$priorities ) {
		...
		print Dumper($id);
		my $objhash = getobjecthash($id);
		print Dumper( $objhash );

		my $identifier = getobjecthash($id)->{'identifier'};
		my $linkstring = $domain->{'urltemplate'} . $identifier;
		my $linkstring = uri_escape( $linkstring );
...
sub matchrest {
	...
	foreach my $title (sort {lc($b) cmp lc($a)} keys %$subhash) {
		...
		while (($i+$midx+$skip+1 < $tlen) && ($midx<$widx)) {
			...
			} elsif (skipword($tlist->[$i+$midx+$skip+1]) ) {
				$skip++;
			} else {
				last;
			}
